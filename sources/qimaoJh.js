...
<body>
  ...
  var CATEGORIES = ["历史", "军事", "游戏", "科幻", "体育", "都市", "玄幻", "武侠", "奇闻", "现实"];
  function api(ctx) {
  ...
  var module = {
    async search(ctx) {
      ...
      const json = await fetchJson(withQuery(joinUrl(api(ctx), "/qimao/search.php"), { wd: keyword, page, pageSize, page_size: pageSize, limit: pageSize, size: pageSize }));
    ...
    async explore(ctx) {
      ...
      const json = await fetchJson(withQuery(joinUrl(api(ctx), "/qimao/qmfx.php"), { fl: category, page, pageSize, page_size: pageSize, limit: pageSize, size: pageSize }));
    ...
    async toc(ctx) {
      ...
      const chapters = (json?.data?.lists || []).map((x, i) => chapterFromItem(ctx.source.id, x, i, { id: x?.url, name: x?.title, chapterUrl: x?.url, extra: x }));
      return { chapters };
    },
    async content(ctx) {
  ...
  var qimaoJh_default = source;

  // .source-build/qimaoJh.ts
  var { module: sourceModule, ...manifest2 } = qimaoJh_default;
  var runtimeModule = {
    ...sourceModule,
    manifest: {
      ...manifest2,
