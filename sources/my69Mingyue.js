...
<body>
  ...
  // src/sources/my69Mingyue.ts
  var BACKEND = "https://no.skybook.qzz.io";
  ...
  function clean69(s) {
    return stripHtml(s).replace(/新..书吧|吧书69新|\(本章完\)|loadAdv\(10,0\);/g, "").trim();
  }
  ...
  var module = {
    async search(ctx) {
      ...
      const json = await fetchJson(withQuery(`${BACKEND}/69/search`, { key: ctx.args.keyword, page: ctx.args.page, pageSize, page_size: pageSize, limit: pageSize, size: pageSize }));
      const books = (json?.list || []).map((x) => toBook(ctx, x));
      return { books, page: ctx.args.page, hasMore: books.length > 0 };
    },
    async explore(ctx) {
      if (!ctx.args.sectionId)
        return { items: rootCategories(["玄幻", "都市", "历史", "科幻", "仙侠", "悬疑", "轻小说", "完本"]) };
      const out = await module.search({ ...ctx, args: { keyword: str(ctx.args.payload?.category || "玄幻"), page: ctx.args.page, pageSize: ctx.args.pageSize } });
    ...
    async toc(ctx) {
      ...
      const json = await fetchJson(withQuery(`${BACKEND}/69/catalog`, { book_id: id }));
      const chapters = (json?.list || []).map((x, i) => chapterFromItem(ctx.source.id, x, i, { id: `${x?.articleid}/${x?.chapterid}`, name: x?.chaptername, chapterUrl: JSON.stringify({ bid: x?.articleid || id, cid: x?.chapterid }) }));
      return { chapters };
    },
    async content(ctx) {
      const { bid, cid } = JSON.parse(ctx.args.chapter.chapterUrl);
      const json = await fetchJson(withQuery(`${BACKEND}/69/content`, { book_id: bid, chapter_id: cid }));
  ...
  var my69Mingyue_default = source;

  // .source-build/my69Mingyue.ts
  var { module: sourceModule, ...manifest2 } = my69Mingyue_default;
  var runtimeModule = {
    ...sourceModule,
    manifest: {
      ...manifest2,
      entry: "my69Mingyue.js"
    }
  };
