...
import { aesCbcPkcs7Base64, bookFromItem, chapterFromItem, fetchJson, makeContent, manifestBase, md5Hex, rootCategories, sortedSign, SOURCE_SETTINGS_COMMON, str, withQuery } from './_common.ts'
...
export async function search(ctx: SearchContext) {
  const params = { gender: '3', imei_ip: '2937357107', page: ctx.args.page, wd: ctx.args.keyword }
...
export async function explore(ctx: ExploreContext) {
  ...
  const out = await search({ ...ctx, args: { keyword: str(ctx.args.payload?.category), page: ctx.args.page } } as any)
