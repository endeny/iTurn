...
import { bookFromItem, chapterFromItem, collectArraysByKey, fetchJson, joinUrl, makeContent, manifestBase, openSettingsPage, rootCategories, SOURCE_SETTINGS_COMMON, str, withQuery } from '../helpers/common.ts'
...
const module: SourceModule = {
  async search(ctx: SearchContext) {
    ...
    const json: any = await fetchJson(withQuery(`${BASE}/search`, { key: keyword, page, pageSize, page_size: pageSize, limit: pageSize, size: pageSize }))
  ...
  async explore(ctx: ExploreContext) {
    ...
    const out = await module.search!({ ...ctx, args: { keyword: str(payload?.category || '玄幻'), page, pageSize: ctx.args.pageSize } } as any)
  ...
  async toc(ctx: TocContext): Promise<any> {
