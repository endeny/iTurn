...
export async function search(ctx: SearchContext) {
  ...
  const json: any = await fetchJson(withQuery(joinUrl(api(ctx), '/shuqi/search.php'), { wd: keyword, page }))
...
export async function explore(ctx: ExploreContext) {
  ...
  const json: any = await fetchJson(withQuery(joinUrl(api(ctx), '/shuqi/sqfx.php'), { fl: str(payload?.category || '玄幻'), page }))
  const items = (json?.data?.books || []).map((x: any) => ({ type: 'book' as const, book: toBook(ctx, x) }))
  return { items, page, hasMore: items.length > 0 }
}
export async function bookInfo(ctx: any): Promise<BookInfo> { return { ...ctx.args.book, tocUrl: ctx.args.book.bookUrl } }
