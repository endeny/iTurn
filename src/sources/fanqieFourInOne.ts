...
import { bookFromItem, chapterFromItem, fetchJson, joinUrl, makeContent, manifestBase, openSettingsPage, rootCategories, SOURCE_SETTINGS_COMMON, str, withQuery } from '../helpers/common.ts'
...
const module: SourceModule = {
  async search(ctx: SearchContext) {
    ...
    const json: any = await fetchJson(withQuery(joinUrl(API, `/${type}/search`), { keyword, page: ctx.args.page, pageSize, page_size: pageSize, limit: pageSize, size: pageSize }), { headers: headers(ctx) })
  ...
  async explore(ctx: ExploreContext) {
    ...
    const fake = { ...ctx, source: { ...ctx.source, settings: { ...ctx.source.settings, type: str(ctx.args.payload?.category || '小说') } }, args: { keyword: str(ctx.args.payload?.category || '热门'), page: ctx.args.page, pageSize: ctx.args.pageSize } } as any
